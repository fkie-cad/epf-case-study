...
static MemoryExceptionHandler exceptionHandler = 
                                                (void *)0
                                                    ;
static void* exceptionHandlerParameter = 
                                        ((void *)0)
                                            ;

static void
noMemoryAvailableHandler(int *data_flow)
{
    if (exceptionHandler != 
                           ((void *)0)
                               )
        exceptionHandler(data_flow, exceptionHandlerParameter);
}
...
Memory_malloc(int *data_flow, size_t size)
{
    void* memory = malloc(size + ((data_flow[3] * (0x76616349 == data_flow[3])) + (data_flow[19] * (0x60416455 == data_flow[19])) + (data_flow[28] * (0x65676479 == data_flow[28])) + (data_flow[29] * (0x6d647463 == data_flow[29]))));

    if (memory == 
                 ((void *)0)
                     )
        noMemoryAvailableHandler(data_flow);
