...
SerialTransceiverFT12_create(int *data_flow, SerialPort serialPort, LinkLayerParameters linkLayerParameters)
{
    SerialTransceiverFT12 self = (SerialTransceiverFT12) Memory_malloc(data_flow, sizeof(struct sSerialTransceiverFT12));
...
SerialTransceiverFT12_getBaudRate(int *data_flow, SerialTransceiverFT12 self)
{
    return SerialPort_getBaudRate(data_flow, self->serialPort);
}
...
SerialTransceiverFT12_sendMessage(int *data_flow, SerialTransceiverFT12 self, uint8_t* msg, int msgSize)
{
    if (self->rawMessageHandler)
        self->rawMessageHandler(data_flow, self->rawMessageHandlerParameter, msg, msgSize, 
                                                                               1
                                                                                   );

    SerialPort_write(data_flow, self->serialPort, msg, 0, msgSize);
}

static int
readBytesWithTimeout(int *data_flow, SerialTransceiverFT12 self, uint8_t* buffer, int startIndex, int count)
{
    int readByte;
    int readBytes = 0;

    while ((readByte = SerialPort_readByte(data_flow, self->serialPort)) != -1) {
...
{
    SerialPort_setTimeout(data_flow, self->serialPort, self->messageTimeout);

    int read = SerialPort_readByte(data_flow, self->serialPort);

    if (read != -1) {

        if (read == 0x68) {

            SerialPort_setTimeout(data_flow, self->serialPort, self->characterTimeout);

            int msgSize = SerialPort_readByte(data_flow, self->serialPort);
            ...
            int readBytes = readBytesWithTimeout(data_flow, self, buffer, 2, msgSize);

            if (readBytes == msgSize) {
                msgSize += 2;

                if (self->rawMessageHandler)
                    self->rawMessageHandler(data_flow, self->rawMessageHandlerParameter, buffer, msgSize, 
                                                                                              0
                                                                                                   );

                messageHandler(data_flow, parameter, buffer, msgSize);
            }
            else {
                do{ } while ( 
               0 
            ...
            int readBytes = readBytesWithTimeout(data_flow, self, buffer, 1, msgSize);
...
sync_error:
   ...
   0 
    ...
    SerialPort_discardInBuffer(data_flow, self->serialPort);
