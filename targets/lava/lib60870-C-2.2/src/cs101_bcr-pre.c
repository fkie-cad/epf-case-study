                                            ...
                                            isInvalid)
{
    if (self == 
        ...
        self = (BinaryCounterReading) Memory_malloc(data_flow, sizeof(struct sBinaryCounterReading));

    if (self != 
               ((void *)0)
                   ) {
        BinaryCounterReading_setValue(data_flow, self, value);
        BinaryCounterReading_setSequenceNumber(data_flow, self, seqNumber);
        BinaryCounterReading_setCarry(data_flow, self, hasCarry);
        BinaryCounterReading_setAdjusted(data_flow, self, isAdjusted);
        BinaryCounterReading_setInvalid(data_flow, self, isInvalid);
    }
