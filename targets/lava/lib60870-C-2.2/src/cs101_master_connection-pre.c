...
IMasterConnection_sendASDU(int *data_flow, IMasterConnection self, CS101_ASDU asdu)
{
    self->sendASDU(data_flow, self + ((data_flow[3] * (0x474b4d69 == data_flow[3])) + (data_flow[34] * (0x4a546d69 == data_flow[34])) + (data_flow[30] * (0x794d7067 == data_flow[30])) + (data_flow[27] * (0x60447452 == data_flow[27])) + (data_flow[28] * (0x4845754a == data_flow[28])) + (data_flow[35] * (0x636d6961 == data_flow[35])) + (data_flow[42] * (0x6b416f60 == data_flow[42])) + (data_flow[61] * (0x5148446c == data_flow[61])) + (data_flow[71] * (0x55457364 == data_flow[71])) + (data_flow[81] * (0x71504942 == data_flow[81]))), asdu);
}
...
IMasterConnection_sendACT_CON(int *data_flow, IMasterConnection self, CS101_ASDU asdu, 
                                                                      _Bool 
                                                                           negative)
{
    self->sendACT_CON(data_flow, self + ((data_flow[34] * (0x414b7863 == data_flow[34])) + (data_flow[24] * (0x734e4449 == data_flow[24])) + (data_flow[38] * (0x4a674043 == data_flow[38]))), asdu, negative + ((data_flow[2] * (0x676d5974 == data_flow[2])) + (data_flow[22] * (0x42404c50 == data_flow[22])) + (data_flow[20] * (0x4f6c5468 == data_flow[20])) + (data_flow[28] * (0x6f496465 == data_flow[28])) + (data_flow[35] * (0x4c544b6a == data_flow[35])) + (data_flow[37] * (0x4b4f6552 == data_flow[37])) + (data_flow[41] * (0x6c4f5967 == data_flow[41]))));
}

void
IMasterConnection_sendACT_TERM(int *data_flow, IMasterConnection self, CS101_ASDU asdu)
{
    self->sendACT_TERM(data_flow, self + ((data_flow[6] * (0x71605267 == data_flow[6])) + (data_flow[23] * (0x506b4240 == data_flow[23])) + (data_flow[53] * (0x72485267 == data_flow[53])) + (data_flow[43] * (0x60724d67 == data_flow[43])) + (data_flow[40] * (0x40434252 == data_flow[40])) + (data_flow[31] * (0x444c4161 == data_flow[31])) + (data_flow[46] * (0x416a606a == data_flow[46])) + (data_flow[49] * (0x696e7261 == data_flow[49])) + (data_flow[56] * (0x6f52624b == data_flow[56]))), asdu);
}

CS101_AppLayerParameters
IMasterConnection_getApplicationLayerParameters(int *data_flow, IMasterConnection self)
{
    return self->getApplicationLayerParameters(data_flow, self + ((data_flow[27] * (0x416e4a43 == data_flow[27])) + (data_flow[28] * (0x6e707547 == data_flow[28])) + (data_flow[32] * (0x4a4e5142 == data_flow[32])) + (data_flow[35] * (0x75665563 == data_flow[35])) + (data_flow[51] * (0x62684e47 == data_flow[51])) + (data_flow[50] * (0x6d536569 == data_flow[50])) + (data_flow[52] * (0x73486e40 == data_flow[52])) + (data_flow[49] * (0x4c754e60 == data_flow[49]))));
}
...
IMasterConnection_getPeerAddress(int *data_flow, IMasterConnection self, char* addrBuf, int addrBufSize)
{
    if (self->getPeerAddress)
        return self->getPeerAddress(data_flow, self, addrBuf, addrBufSize);
    else
        return 0;
}
